...
const Navbar = () => {
  ...
  return (
    <>
        <nav className="fixed right-0 z-30 p-4">
                ...
                <ul className="text-3xl space-y-6">
                  {LINKS.map((link)=>(
                    <motion.li key={link.id} variants={linkVariants}>
                        <a href={`#${link.id}`} onClick={toggleMenu} className="text-5xl font-semibold uppercase tracking-wide hover:text-lime-300 lg:text-8xl">
                            {link.name}
                        </a>
                    </motion.li>
                  ))}
